...
void Game::draw_loading(int round)
{
  al_clear_to_color(al_color_name("black"));
  std::string text = "";
  std::stringstream ss_rounds;
  std::stringstream ss_m_rounds;
  ss_rounds << round;
  ss_m_rounds << m_rounds;
  if (m_rounds > 1)
    text = "Round " + ss_rounds.str() + " of " + ss_m_rounds.str();
...
void Game::show_current_standing(bool hide_standing, bool game_over)
{
  ...
  if (!hide_standing && m_num_snakes > 1)
  {
    ...
    {
	  ss << (player_rankings->front()+1);
      text = "Player " + ss.str() + " Wins!";
    ...
    for (int i = 0; i < m_num_snakes; i++)
    {
	  ...
	  text += "Player " + ss.str() + " with ";
	  ss.str("");
	  ss << m_player_scores[player_rankings->at(i)];
      text += ss.str();
