...
const int MEDIUM_MAX_NOTICE_PELLET = 20;
...
Input::Direction AI::find_safest_direction()
{
  ...
  {
    case Input::LEFT:
      max_moves = get_max(left_moves, get_max(down_moves, up_moves));
      if (max_moves == left_moves)
        return Input::LEFT;
      else if (down_moves == up_moves)
        return get_random_turn_direction();
      else if (max_moves == down_moves)
        return Input::DOWN;
      else if (max_moves == up_moves)
        return Input::UP;
      else
        assert(false);
      break;
    case Input::RIGHT:
      max_moves = get_max(right_moves, get_max(down_moves, up_moves));
      if (max_moves == right_moves)
        return Input::RIGHT;
      else if (down_moves == up_moves)
        return get_random_turn_direction();
      else if (max_moves == down_moves)
      ...
      break;
    case Input::DOWN:
      max_moves = get_max(down_moves, get_max(left_moves, right_moves));
      if (max_moves == down_moves)
        return Input::DOWN;
      else if (left_moves == right_moves)
        return get_random_turn_direction();
      else if (max_moves == left_moves)
        return Input::LEFT;
      else if (max_moves == right_moves)
        return Input::RIGHT;
      else
        assert(false);
      break;
    case Input::UP:
      max_moves = get_max(up_moves, get_max(left_moves, right_moves));
      if (max_moves == up_moves)
        return Input::UP;
      else if (left_moves == right_moves)
        return get_random_turn_direction();
      else if (max_moves == left_moves)
