...
Snake::Snake(int player_num, float x, float y, float volume, Input::Direction direction, int size, ALLEGRO_COLOR color, float width, float max_x, float max_y, bool wrap, Collision_Table *collision_table, bool tron)
{
  ...
  std::stringstream ss;
  ss << random;
  m_dead_sound = al_load_sample(("sounds\\squish" + ss.str() + ".wav").c_str());
  m_eat_sound_speed = 0.7 + ((float)player_num * 0.2);

  //Head
  m_pieces->push_back(new Snake_Piece(direction, Snake_Piece::HEAD, new Simple_Rectangle(x, y, m_width, m_width, true, m_color)));
  m_collision_table->insert(x, y, this);
  for (int i = 1; i < m_size; i++)
  { //Body
    Simple_Rectangle *rectangle = this->create_rectangle(direction, x, y, i);
...
void Snake::move()
{
  ...
  {
    m_collision_table->get_pellet(front_x, front_y)->eat(this);
  }
  m_pieces->front()->draw();
  m_collision_table->insert(front_x, front_y, this);
  //Move the body
  for (std::vector<Snake_Piece*>::iterator iter = std::next(m_pieces->begin()); iter != m_pieces->end(); ++iter)
  {
    ...
    if ((*iter) == m_pieces->back()) //tail
    {
      if (m_tron || m_grow > 0)
      {
        ...
        Simple_Rectangle *rectangle = this->create_rectangle(tmp_direction, prev_x, prev_y, m_size-1);
...
Simple_Rectangle *Snake::create_rectangle(Input::Direction direction, float x, float y, int position)
{
  Simple_Rectangle *rectangle = NULL;
  switch(direction)
  {
    case Input::LEFT:
      rectangle = new Simple_Rectangle(x+(position*m_width), y, m_width, m_width, true, m_color);
    ...
    case Input::RIGHT:
      ...
      break;
    case Input::DOWN:
      rectangle = new Simple_Rectangle(x, y-(position*m_width), m_width, m_width, true, m_color);
