...
Scoreboard::Scoreboard(float screen_width, float screen_height, float snake_width, float font_small_incrementor, ALLEGRO_FONT *font_small, float y, int num_snakes, int player_scores[], Snake *snakes[])
{
  ...
  for (int i = 0; i < m_num_snakes; i++)
  {
    ...
    m_backgrounds[i] = new Simple_Rectangle(x, y, m_width/m_num_snakes, m_height, true, color, m_snake_width/5);
...
void Scoreboard::draw_player_score(Snake *snake)
{
  ...
  std::stringstream ss;
  ss << m_player_scores[player_num];
  al_draw_text(m_font, text_color, m_text_x[player_num], m_y + m_font_small_incrementor, ALLEGRO_ALIGN_CENTER, ss.str().c_str());
}
